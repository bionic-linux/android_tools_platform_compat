...
public class ChangeIdProcessing {
    ...
    public ChangeIdProcessing(ProcessingEnvironment processingEnv, RoundEnvironment roundEnv) {
        mProcessingEnv = processingEnv;
        mRoundEnv = roundEnv;
    }

    public boolean process() {
        Set<? extends Element> annotatedElements = mRoundEnv.getElementsAnnotatedWith(
        ...
        for (Element e : annotatedElements) {
            if (!isValidChangeId(e)) {
                continue;
            }
            Change change = createChange(e, mProcessingEnv.getElementUtils().getDocComment(e));
        ...
        for (Map.Entry<PackageClass, XmlWriter> entry : writersByClass.entrySet()) {
            ...
            try (OutputStream output = mProcessingEnv.getFiler().createResource(
                    CLASS_OUTPUT,
                    key.javaPackage,
     ...
     * operations are safe.
     */
    private boolean isValidChangeId(Element e) {
    ...
    private String getSourcePosition(Element e, AnnotationMirror a) {
        JavacElements javacElem = (JavacElements) mProcessingEnv.getElementUtils();
    ...
    private Change createChange(Element e, String comment) {
