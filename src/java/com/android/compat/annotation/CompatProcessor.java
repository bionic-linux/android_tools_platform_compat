...
/**
 * Annotation processor for {@link ChangeId} annotations.
 *
 * <p>This processor outputs an XML file containing all the changeIds defined by this
 * annotation. The file is bundled into the pratform image and used by the system server.
 * Design doc: go/gating-and-logging.</p>
 */
@SupportedAnnotationTypes({"android.compat.annotation.ChangeId"})

public class CompatProcessor extends AbstractProcessor {
    ...
    private static final String IGNORED_CLASS = "Compatibility";
    ...
    private boolean shouldIgnoreAnnotation(Element e) {
        ...
        return (e.getKind() == ElementKind.PARAMETER &&
                ...
                e.getEnclosingElement().getEnclosingElement().getSimpleName().toString().equals(
                        IGNORED_CLASS));
    }
     ...
     * safe.
     */
    boolean isValidChangeId(Element e, Messager messager) {
        ...
        if (((VariableElement) e).asType().getKind() != TypeKind.LONG) {
            messager.printMessage(
                    ERROR,
        ...
        for (AnnotationMirror m : e.getAnnotationMirrors()) {
            ...
            if (type.equals(DISABLED_CLASS_NAME)) {
                ...
                for (Map.Entry<?, ?> entry : m.getElementValues().entrySet()) {
                    if (((ExecutableElement) entry.getKey()).getSimpleName().toString().equals(
                            TARGET_SDK_VERSION)) {
